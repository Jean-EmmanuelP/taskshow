...
const Navbar = () => {
    ...
    try {
      ...
      if (currentUser && userId) {
        const logoutLogData = {
          ...
          role: userRole,
          action: "logout",
          loginTime: null, // Will be filled from existing log if needed
          logoutTime: new Date().toISOString(),
          ipAddress: "127.0.0.1", // In production, this would be captured from the request
          tokenName: "logged_out"
        };
        
        // Store logout log in localStorage for admin view
  ...
  const handleLogoClick = (e) => {
    ...
    // If user is not authenticated, just navigate to home
    if (!isAuthenticated) {
      navigate("/");
      return;
    }
  ...
  return (
    <nav className="bg-blue-600 text-white px-5 py-4 flex justify-between items-center shadow-lg">
      ...
      <div className="flex items-center gap-4">
        {/* Task List Button (Only shown when authenticated and not on auth routes) */}
        ...
        {/* Profile Button (Only shown when authenticated and not on auth routes) */}
        {isAuthenticated && !hideProfileRoutes.includes(location.pathname) && (
